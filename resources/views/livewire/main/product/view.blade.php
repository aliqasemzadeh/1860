<x-slot name="title">
    @php
        ...
        if ($this->product->brand) {
            $seoTitle .= ' - ' . $this->product->brand->name;
        }
        if ($this->product->category) {
            $seoTitle .= ' | ' . $this->product->category->name;
        }
    @endphp
    {{ $seoTitle }}
</x-slot>
<x-slot name="head">
    @php
        ...
        $offers = null;
        if ($selectedPrice) {
            $finalPrice = ($selectedPrice->sale_price && $selectedPrice->sale_price < $selectedPrice->price)
                ? $selectedPrice->sale_price
                : $selectedPrice->price;
            $availability = $selectedPrice->quantity > 0
            ...
            $offers = [
                ...
                'url' => $canonicalUrl,
                'priceCurrency' => 'IRR',
                'price' => (string) ((float) $finalPrice * 10),
    ...
    @if($selectedPrice && $selectedPrice->quantity > 0)
        <meta name="product_price" content="{{ $selectedPrice->price }}">
        <meta name="product_old_price" content="{{ $selectedPrice->sale_price }}">
        <meta name="availability" content="instock">
        <meta name="guarantee" content="{{ $selectedPrice->warranty->name ?? $selectedPrice->warranty->title ?? '' }}">
    @elseif($selectedPrice && $selectedPrice->quantity <= 0)
