    ...
    <section class="py-8 antialiased md:py-12">
        <div class="mx-auto max-w-7xl px-4 2xl:px-0">
            ...
            <nav class="mb-6 flex items-center gap-2 text-sm text-zinc-600 dark:text-zinc-400">
                ...
                <span>/</span>
                <span class="text-zinc-900 dark:text-zinc-100 font-medium">{{ __('app.checkout') }}</span>
            </nav>

            {{-- Page Header --}}
            <div class="mb-8">
                <flux:heading size="xl">{{ __('app.checkout') }}</flux:heading>
                <flux:text class="mt-2 text-zinc-600 dark:text-zinc-400">{{ __('app.checkout_description') }}</flux:text>
            </div>
                ...
                @else
                    <form wire:submit="verify" class="space-y-8">
                        <div class="space-y-2">
                            ...
                            <flux:text class="text-center text-zinc-600 dark:text-zinc-400">{{ __('app.mobile_confirm_question') }}</flux:text>
                        </div>
                        <flux:field>
                            <flux:label>{{ __('app.otp_code_label') }}</flux:label>
                        ...
                        <div class="space-y-4">
                            ...
                            <flux:button type="button" wire:click="resend" variant="ghost" class="w-full">{{ __('app.resend_code') }}</flux:button>
                        </div>
                    </form>
                @endif
            </div>
