...
<div>
    <div class="mx-auto max-w-7xl px-4 2xl:px-0 py-6 md:py-10">
        <flux:breadcrumbs class="mb-6">
            ...
            <flux:breadcrumbs.item>{{ __('general.blog') }}</flux:breadcrumbs.item>
        </flux:breadcrumbs>
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div class="space-y-3 max-w-2xl">
                    <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold bg-teal-50 text-teal-700 dark:bg-teal-950/60 dark:text-teal-300 border border-teal-200/80 dark:border-teal-800/60">
                        <flux:icon.newspaper size="xs" />
                        <span>{{ __('general.articles') }}</span>
                    </div>
                    <flux:heading size="2xl" level="1">{{ __('general.blog') }}</flux:heading>
                    <flux:subheading size="lg" class="text-zinc-600 dark:text-zinc-300">{{ __('general.blog_description') }}</flux:subheading>
                </div>
        ...
        @if ($this->posts->isNotEmpty())
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                @foreach ($this->posts as $post)
                    <article wire:key="post-card-{{ $post->id }}" class="group relative flex flex-col rounded-3xl bg-white dark:bg-zinc-900 border border-zinc-200/80 dark:border-zinc-800 overflow-hidden shadow-sm hover:shadow-xl dark:hover:shadow-zinc-950/60 transition-all duration-300 hover:-translate-y-1.5">
                        <a href="{{ $post->url }}" wire:navigate class="block relative aspect-[16/10] overflow-hidden bg-zinc-100 dark:bg-zinc-800">
                            ...
                            @if ($post->tags->isNotEmpty())
                                <div class="absolute top-3 right-3 flex flex-wrap gap-1">
                                    <span class="px-2.5 py-1 text-xs font-medium rounded-full bg-white/90 dark:bg-zinc-900/90 text-zinc-800 dark:text-zinc-200 backdrop-blur-md shadow-sm border border-white/20 dark:border-zinc-700/50">
                        ...
                        <div class="flex flex-col flex-grow p-6">
                            ...
                            @if ($post->tags->count() > 1)
                                <div class="flex flex-wrap gap-1.5 mb-5">
                                    @foreach ($post->tags->skip(1)->take(3) as $tag)
