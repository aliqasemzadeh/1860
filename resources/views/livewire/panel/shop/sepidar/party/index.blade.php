                ...
                <flux:table.cell class="w-1 whitespace-nowrap">
                    <div class="flex items-center gap-2">
                        <flux:button size="xs" variant="primary" color="sky"
                                     wire:click="$dispatch('panel.shop.sepidar.party.addresses.assign-data', { id: '{{ $customer->id }}' })">{{ __('app.address') }}</flux:button>
                        <flux:button size="xs" variant="primary" color="green"
                                     wire:click="$dispatch('panel.shop.sepidar.party.invoices.assign-data', { id: '{{ $customer->id }}' })">{{ __('app.invoices') }}</flux:button>
                        <flux:button size="xs" variant="primary" color="red"
                                     wire:click="$dispatch('panel.shop.sepidar.party.phones.assign-data', { id: '{{ $customer->id }}' })">{{ __('app.phones') }}</flux:button>
                    </div>
                </flux:table.cell>
