<flux:modal name="panel.shop.shipping.rate.create.modal" class="md:w-[36rem]" flyout position="right">
    <div class="space-y-6">
        <div>
            <flux:heading size="lg">{{ __('app.create_shipping_rate') }}</flux:heading>
            <flux:text class="mt-2">{{ __('app.create_shipping_rate_description') }}</flux:text>
        </div>
        ...
        <form wire:submit="create" method="post">
            <div class="pb-2 space-y-3">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <flux:field>
                        <flux:label>{{ __('app.min_weight') }} ({{ __('general.gram') }})</flux:label>
                        <flux:input wire:model="min_weight" type="number" min="0" step="1" />
                        <flux:error name="min_weight" />
                    </flux:field>

                    <flux:field>
                        <flux:label>{{ __('app.max_weight') }} ({{ __('general.gram') }})</flux:label>
