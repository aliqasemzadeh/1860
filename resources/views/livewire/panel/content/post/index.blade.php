<div>
    <div class="relative mb-6 w-full">
        <div class="flex items-center justify-between">
            <div>
                <flux:heading size="xl" level="1">{{ __('app.posts') }}</flux:heading>
                <flux:subheading size="lg" class="mb-6">{{ __('app.posts_description') }}</flux:subheading>
            </div>
            @can('content_post_create')
                <flux:modal.trigger name="panel.content.post.create.modal">
                    <flux:button variant="primary" color="teal" icon="plus">{{ __('app.create_post') }}</flux:button>
                </flux:modal.trigger>
            @endcan
        </div>
    ...
    <flux:table :paginate="$this->posts">
        <flux:table.columns>
            ...
            <flux:table.column>{{ __('app.related_products') }}</flux:table.column>
            ...
            <flux:table.row :key="$post->id">
                ...
                <flux:table.cell class="whitespace-nowrap">
                    @if ($post->status === 'published')
                        <flux:badge color="green" size="sm">{{ __('app.post_status_published') }}</flux:badge>
                    @elseif ($post->status === 'archived')
                        <flux:badge color="zinc" size="sm">{{ __('app.post_status_archived') }}</flux:badge>
                    @else
                        <flux:badge color="amber" size="sm">{{ __('app.post_status_draft') }}</flux:badge>
                    @endif
                </flux:table.cell>
                <flux:table.cell>
