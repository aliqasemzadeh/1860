        ...
        <div class="flex items-center justify-between">
            ...
            @can('accounting_bank_transaction_create')
                <flux:modal.trigger name="accounting.bank.transaction.create.modal">
                    <flux:button variant="primary">{{ __('app.create_transaction') }}</flux:button>
                </flux:modal.trigger>
            @endcan
        </div>
            ...
            <flux:table.column colspan="8" class="bg-white dark:bg-zinc-900">
                <div class="flex flex-row gap-2 pe-2 items-end">
                    ...
                    <flux:select wire:model.live="typeFilter" size="sm" placeholder="{{ __('app.filter_transaction_type') }}">
                        ...
                        @foreach (\App\Enums\TransactionTypeEnum::options() as $type)
                ...
                <flux:table.row :key="$transaction->id">
                    ...
                    <flux:table.cell>
                        <span class="{{ $transaction->amount >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">
                            {{ $transaction->amount >= 0 ? '+' : '-' }}{{ number_format(abs($transaction->amount), 0) }} {{ __('app.toman') }}
                        </span>
                    </flux:table.cell>
                    <flux:table.cell>
